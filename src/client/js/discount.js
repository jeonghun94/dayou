...
const DESK_ID = process.env.DESK_ID.replace(/ /gi, "").split(",");
...
function searchInSeqNo(inSeqNo, cloneNode) {
  fetch(`/discount/search/inseqno`, {
    ...
    .then((data) => {
      ...
      if (DESK_ID.includes(id)) {
        console.log(companys);
        companyList(companys);
      }
      //할인권 리스트 추가
      couponList(freeCouponList, "할인", "01");
      couponList(payCouponList, "할인", "02");
...
function discountList(list, inSeqNo) {
  if (list.length !== 0) {
    ...
    list.map((x) => {
      ...
      discountInfoShopName.innerText = `등록계정: ${x.id}`;

      const discountInfoCompanyName = document.createElement("p");
      discountInfoCompanyName.innerText = `요청부서: ${x.companyName}`;
      ...
      const id = document.querySelector("#crudArticle").dataset.id;
      if (DESK_ID.includes(id)) {
        discountInfo.appendChild(discountInfoCompanyName);
      }
...
function companyList(companys) {
  ...
  select.id = "company";
  select.name = "company";
  select.innerHTML = `<option value="00">전체</option>`;
...
function couponList(list, text, payType) {
  if (list.length !== 0) {
    ...
    select.addEventListener("change", (e) => {
      ...
      let companyNo;
      if (DESK_ID.includes(id)) {
        const companySelect = document.querySelector("#company").value;
      ...
      if (
        ...
        let reason;
        if (DESK_ID.includes(id)) {
          reason = window.prompt("사유를 입력해주세요.");
          const pattern = /\s/g;
          if (
            reason === null ||
            reason === "" ||
        ...
        insertList(inSeqNo, couponType, type, companyNo, reason);
      } else {
        resetSelect();
      }
    });
...
// 웹 할인 등록
async function insertList(inSeqNo, couponType, select, companyNo, reason) {
  if (couponType !== "00") {
