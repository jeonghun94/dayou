...
const checkMasterId = async (id) => {
  const masterId = process.env.MASTER_ID.replace(/ /gi, "").split(",");
  console.log(masterId);
  let userPosition = 2;

  if (masterId.includes(id)) {
    userPosition = 1;
  }
  const [{ auth }] = await executeQuery(`SELECT  Auth
                                          FROM employeeauth 
...
export const searchInSeqNo = async (req, res) => {
  ...
  const auth = await checkMasterId(id);

  const payCouponList = [];
  const freeCouponList = await executeQuery(
    `select * from ps132 where coupontype in (${auth})`
  );
  const discountList = await executeQuery(
    DISCOUNT_QUERY.SEARCH_DISCOUNT_LIST(inSeqNo)
  );

  const companys = await executeQuery(DISCOUNT_QUERY.COMPANYS());
...
export const insertList = async (req, res) => {
  ...
  if (req.body.companyNo) {
    obj.company = req.body.companyNo;
  }
...
export const deleteList = async (req, res) => {
  ...
  console.log(`{ registerCode: ${registerCode}, id: ${id} }`);
